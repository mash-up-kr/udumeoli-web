...
// "pot-ut-1" 하드코딩과 같은 컨벤션). 이 팟의 멤버는
// user-1 + m-유지-0/m-성아-1/m-가연-2/m-우두머리대장-3.
...
// 창원 최신 방문에 나(user-1)·성아·우두머리대장이 빠져 있어 '기록하기' CTA와 zzZ 슬롯,
// 카드 Alert, 지도 미완료(협업) 상태를 모두 확인할 수 있다.
// 표시 엣지 케이스 시드: 타인 사진 코멘트 없음(대전 06-20 우두머리대장),
// 최대 길이(40자) 코멘트(창원 05-03 우두머리대장), 최대 길이(6자) 닉네임 zzZ(창원 최신 방문).
type AlbumSeed = {
  region: string
...
const ALBUM_SEEDS: Array<AlbumSeed> = [
  {
    ...
    keyword: "FOOD",
    uploads: [
      // 최신 방문 — 유지·가연만 업로드, 나·성아·우두머리대장 미기록 (기록하기/zzZ 케이스)
      ...
      ["2026-05-03", "m-가연-2"],
      // 코멘트 최대 길이(40자) 표시 케이스 — 타일에서 2줄 말줄임 확인용
      [
        "2026-05-03",
        "m-우두머리대장-3",
        "빵 냄새 가득한 골목에서 하루 종일 걷고 먹고 또 먹었던 최고의 여행!",
      ],
      ["2026-03-14", "user-1"],
  ...
  },
  {
    ...
    uploads: [
      ...
      ["2025-07-19", "m-우두머리대장-3", "물회가 진짜 미쳤음"],
    ],
  },
]

...
function BottomNav({
  ...
  return (
    <nav
      ...
      {...props}
    >
      <div className="flex items-center justify-between rounded-full bg-bg-neutral-inverse px-8 py-3">
        ...
        <button
          ...
          className={cn(
            "flex w-[60px] flex-col items-center gap-1 py-1",
        ...
        >
          ...
          <span className="text-[10px] leading-[1.5] font-semibold tracking-[-0.2px]">
            여행 앨범
          </span>
        </button>
        <button
          ...
          className="flex w-[60px] flex-col items-center gap-1 py-1 text-fg-neutral-subtle"
        >
      ...
      {/* 중앙 지구본(시안 2885-8192, 96×96 래스터라 2x PNG) — 바 상단 위로 35.36px 돌출(시안 y:-35.36).
          핸들러 주입 시 지도 이동 버튼 */}
      {onGlobeClick ? (
        <button
          type="button"
          ...
          className="absolute top-[-35.36px] left-1/2 -translate-x-1/2"
        >
        ...
        <img
          src={globeSrc}
          alt=""
          className="pointer-events-none absolute top-[-35.36px] left-1/2 size-[96px] -translate-x-1/2"
        />
      )}
    </nav>
  )
